...
class Main extends React.PureComponent {
  ...
  nextSong = () => {
    let next_song;
    const index = this.props.songs.findIndex(
      (i) => i.id === this.state.current_song
    );
  ...
  shuffle = () => {
    this.playSong(
      Math.floor(Math.random() * Math.floor(this.props.songs.length))
    );
  };
  //  Toggle mute
  toggleMute = () => {
    ...
    return (
      <Container>
        <Playlist
          ...
          audio={this.audio}
        />
        {this.props.songs.length >= 0 && !this.state.removed ? (
          <Player
